...
impl CoreManager {
    ...
    /// Path of the encrypted site-profile store, beside the vault.
    ///
    /// A separate file specifically to avoid touching the vault's bincode layout:
    /// bincode is positional, so an extra field on `NoteList` would break every
    /// existing vault. Encrypted with the same cascade and cached password, so the
    /// list of sites the user holds accounts with is never on disk in the clear.
    fn site_profiles_path(&self) -> PathBuf {
        self.data_path.with_file_name("site_profiles.dat")
    }

    /// Decrypt and return the saved site profiles. An absent or unreadable file
    /// yields an empty set rather than an error — a corrupt profile store must
    /// never stop the app from opening the notes.
    pub fn load_site_profiles(&self) -> SiteProfiles {
        let path = self.site_profiles_path();
        let Ok(raw) = std::fs::read(&path) else {
            return SiteProfiles::default();
        };
        let Some(state) = CRYPTO_STATE.get() else {
            return SiteProfiles::default();
        };
        ...
        let decrypted = if cascade::is_cascade(&raw) {
            cascade::decrypt_vault(crypto.password.as_slice(), &raw).ok()
        } else {
            EncryptedData::from_bytes(&raw)
                .ok()
                .and_then(|d| crypto::decrypt(&crypto.key, &d).ok())
        };
        decrypted
    ...
    /// Encrypt and write the site profiles. Requires an unlocked vault, since the
    /// encryption reuses the cached master password.
    pub fn save_site_profiles(&self, profiles: &SiteProfiles) -> Result<()> {
        let serialized = bincode::serialize(profiles)?;
        let encrypted = self.encrypt_blob(&serialized)?;
        // Same M1/M2 treatment as the vault: 0600 (with a custom DB path the
        // parent dir is not our 0700 app dir) and atomic, so a crash can't leave
        // a corrupt store that load_site_profiles would silently replace with an
        // empty set on the next save.
        Self::write_vault_atomic(&self.site_profiles_path(), &encrypted)?;
        Ok(())
    }
    ...
    pub fn unlock(&mut self, master_password: MasterPassword) -> Result<()> {
        ...
        // Deliberately NO early-return when already unlocked: if an unlock prompt
        // ever races the auto-lock, accepting a password without checking it would
        // let a wrong password "succeed". Fall through to the normal decrypt path,
        // which fails on a wrong password and reinstalls fresh state on success.
    ...
    pub fn export_all_encrypted(&self, export_path: &PathBuf) -> Result<()> {
        let bytes = self.encrypt_current()?;
        // 0600 + atomic, like the vault itself. The user can loosen the mode if
        // they mean to share the export; defaulting it world-readable is not ours
        // to decide.
        Self::write_vault_atomic(export_path, &bytes)?;
        Ok(())
    }
